...
> **🚀 BEFORE YOU BEGIN**
>
> 1. **Verify your private key is set**: Run `npx chainai@latest who-am-i` to confirm `CHAINAI_PRIVATE_KEY` is properly configured and resolves to the expected address before proceeding with any other command.
> 2. **No private key?** Use `npx chainai@latest gen-wallet` to generate a new wallet. Save the returned private key securely (e.g. as an environment variable) — it will not be shown again.
...
```bash
npx chainai@latest who-am-i -k 0xKEY
# or
CHAINAI_PRIVATE_KEY=0xKEY npx chainai@latest who-am-i
```
...
npx chainai@latest sign-message -k 0xKEY -m 0x68656c6c6f --raw
```
...
```bash
npx chainai@latest sign-typed-data -k 0xKEY -d '<json>'
```
...
```bash
npx chainai@latest sign-transaction -k 0xKEY -t '<json>'
```
...
```bash
npx chainai@latest get-balance -a 0xADDRESS
npx chainai@latest get-balance -a 0xADDRESS -n bsc
npx chainai@latest get-balance -a 0xADDRESS -t 0xTOKEN_CONTRACT
npx chainai@latest get-balance -a 0xADDRESS --all
```
...
```bash
npx chainai@latest send -k 0xKEY --to 0xRECIPIENT --amount 1.5 -t 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
npx chainai@latest send -k 0xKEY --to 0xRECIPIENT --amount 100 -t 0xTOKEN_CONTRACT -n bsc
```
...
```bash
npx chainai@latest swap -k 0xKEY --from-token 0xFROM --to-token 0xTO --amount 1.5
npx chainai@latest swap -k 0xKEY --from-token 0xFROM --to-token 0xTO --amount 1.5 -y
npx chainai@latest swap -k 0xKEY --from-token 0xFROM --to-token 0xTO --amount 0.5 -n bsc
```
...
npx chainai@latest swap-order-status -k 0xKEY --order-hash 0xORDER_HASH -n bsc
```
